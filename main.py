...
@app.post("/items/")
async def create_item(item: Item):  # post 매서드용 요청 (create)
    # BaseModel은 데이터 모델링을 쉽게 도와주고 유효성 검사도 수행
    # 잘못된 데이터가 들어오면 422 오류코드를 반황
    return item


@app.get("/")   # 웹 브라우저에 http://loacalhost:8001/ -> get 요청시 처리
async def read_root():
    return {"Hello": "World"}


@app.get("/item/{item_id}")       # http://localhost:8001/item/1 -> get 요청
